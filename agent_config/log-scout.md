...
| Simple content search | **Grep** | `output_mode: "content"`, `-C 2` |
| Complex regex, multi-pattern, file-type filter | **Bash + rg** | See `skills/ripgrep.md` |
| Extract line range from large file | **Bash** | `sed -n '45,67p' file.log` |
| Filter/summarize large file (>500 lines) | **ctx_execute_file** | See `skills/context-mode.md` |
| Run command with verbose output | **ctx_execute** | See `skills/context-mode.md` |
| Semantic search across indexed content | **ctx_search** | See `skills/context-mode.md` |
| Small files only (<100 lines) | **Read** | — |

**ALWAYS** exclude `/vendor` and `/target` from searches.
...
# Forbidden
- NEVER read /vendor or /target directories
